...
![설치 기사가 방을 가득 채운 메인프레임 옆에 서 있고, 매뉴얼을 든 프로그래머는 난감한 표정을 짓는다.](images/ko_image10.png)
> "설치끝!" \
> "어떻게 프로그래밍을 해야하나?"

초기 컴퓨팅에서 수학은 중심적인 역할을 했고, 자연스럽게 수학을 전공하는 학생들이 프로그래밍을 시작하는 경우가 많았다.

![수학자가 칠판에 수식을 쓰고, 한 프로그래머가 옆에서 기다린다.](images/ko_image8.png)
...
![데니스 리치가 뒤에서 지켜보는 가운데 한 프로그래머가 메인프레임 앞의 콘솔에서 작업한다.](images/ko_image2.png)
> "수학 보다 훨 재미있는걸"

C와 유닉스를 만든 데니스 리치는 대학에서 물리학과 응용수학을 공부했다.


1960년대 아폴로11 프로젝트에 참여하여 달착륙선 소프트웨어를 개발한 [마거릿 해밀턴](https://ko.wikipedia.org/wiki/마거릿_해밀턴_\(과학자\))의 이야기를 들어보면 그 당시 소프트웨어 개발 환경과 인식을 잘 알 수 있다. 대학에서 수학을 공부한 그녀는 남편의 박사 학위 공부를 지원하기 위해 MIT에서 프로그래머로 일을 시작했다.
...
![마거릿 해밀턴이 초기 프로그래머들의 교육 환경을 설명하며 손을 들어 보인다.](images/ko_image5.png)
> "당시에는 컴퓨터 과학과 소프트웨어 공학의 정규 교육과정이 아니었죠. 그래서 프로그래머들은 실무 경험을 통해 일을 배워야 했어요."

해밀턴은 시스템 프로그래밍 전문가가 된 뒤 MIT Instrumentation Laboratory에 합류했다. 그곳에서 아폴로 사령선과 달 착륙선의 탑재 비행 소프트웨어를 개발한 Software Engineering Division을 이끌었다. 당시 아폴로11 프로젝트에는 소프트웨어 개발을 위한 예산과 일정이 포함되어 있지 않았고, 요구 사항에도 소프트웨어 대한 언급은 없었다[1]. 하지만, 소프트웨어는 우주선 비행과 달착륙선을 제어할 만큼 중요한 역할을 맡았고, 마거릿은 좀 더 신뢰성 높은 소프트웨어를 개발하기 위해 딸을 데리고 주말에 출근하기도 했다.

![마거릿 해밀턴이 책상에서 일하고, 딸은 새턴 V 로켓 모형 옆에서 기다린다.](images/ko_image7.png)
> "엄마 집에 언제가요?" \
> "다 끝났어 이제 가자"

결국, 1968년에는 무려 400여명의 사람이 달착륙선 프로그램 개발에 참여하였고[1], 1969년 아폴로 11호는 달 착륙에 성공했다.

![우주선이 지구와 달 사이를 오가는 경로 아래에서 마거릿 해밀턴이 두 팔을 들고 기뻐한다.](images/ko_image4.png)
> "달착륙 성공!"
...
![마거릿 해밀턴이 한 손가락을 들어 새 분야의 이름을 강조한다.](images/ko_image12.png)
> "소프트웨어 공학!"
...
![위에서는 여성이 대형 컴퓨터 콘솔에서 일하고, 아래에서는 두 남성 하드웨어 엔지니어가 큰 설계도를 검토한다.](images/ko_image1.png)
> "하는 일에 비해 월급이 너무 작아"

에니악에 처음 배정된 프로그래머 여섯 명은 모두 여성이었다[3].

![두 여성이 에니악 제어판에 케이블을 연결하고, 한 사람은 작업 지시가 적힌 종이를 들고 있다.](images/ko_image11.png)
...
![그레이스 호퍼가 작동하지 않는 하버드 Mark II 옆에서 양손을 허리에 얹고 서 있다.](images/ko_image3.png)
> "왜 입력이 안되지?"

Mark II 팀은 기계의 릴레이 가운데 하나에 나방이 끼어 있는 것을 발견했다.

![그레이스 호퍼가 하버드 Mark II의 릴레이 사이에 끼인 나방을 가리킨다.](images/ko_image9.png)
> "릴레이 안에 나방이 죽어 있네"

그리고 나방을 로그북에 붙이고 `First actual case of bug being found`라고 적었다. 기술적 결함을 뜻하는 `bug`라는 말은 이 사건보다 훨씬 전부터 쓰였지만, 호퍼와 Mark II 팀은 컴퓨팅 분야에서 `bug`와 `debugging`이라는 말을 널리 알리는 데 기여했다[5].

![그레이스 호퍼가 나방을 붙인 로그북에 메모를 쓴다.](images/ko_image6.png)
> "최초의 컴퓨터 버그를 찾다."

이처럼 여성들은 초기 프로그래밍에서 선구자적인 역할을 했고, 소프트웨어 공학의 토대를 세우는 데 기여했다.

![마거릿 해밀턴이 높이 쌓인 아폴로 비행 소프트웨어 출력물 옆에 서 있다.](images/margaret_hamilton.png)
