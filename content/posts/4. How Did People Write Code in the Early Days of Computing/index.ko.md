...
![두 기술자가 서로 다른 대형 계산 장치의 제어판을 가리키고, 아래에서는 한 사람이 턱을 괸 채 탄도 계산용 컴퓨터를 구상한다.](images/4_1_ko.png)
> "이건 사칙연산을 위한 계산기" \
> "독일군 암호 해독을 위한 장치" \
> "탄도 계산하는 장치를 만들어 볼까?" \
> "이번에는 릴레이가 몇개 필요할까? 진공관도 필요하겠군"

에니악(ENIAC)이 개발되면서 프로그램을 재구성할 수 있는데, 플러그판(plugboard)과 스위치를 연결해서 물리적으로 프로그램을 구성하고 데이터와 명령어를 입력하는 방식이였다. 당연히 매우 번거로운 작업이였고, 프로그램을 변경하려면 배선을 다시해야했다[&lbrack;1&rbrack;][1][&lbrack;4&rbrack;][4].
...
![검은 안경을 쓴 프로그래머가 길게 늘어선 이진수를 보고 놀란다.](images/4_2_ko.png)

그래서 컴퓨터 프로그래밍의 초기부터 어셈블리어의 초기 형태가 등장했다. 숫자로 된 기계 명령 대신 니모닉(mnemonic)이라는 짧은 기호를 사용해 명령을 더 쉽게 표현한 것이다. 에드삭 프로그래머들은 한 글자로 된 오더 코드를 사용했고, Initial Orders라는 작은 부트스트랩 프로그램이 종이테이프에서 이 코드를 읽어 기계 명령으로 변환해 메모리로 로딩했다[&lbrack;2&rbrack;][2].
...
![두 개의 17비트 EDSAC 명령을 동작 코드, 사용하지 않는 비트, 피연산자 주소, 길이 비트로 나누고 T0S와 H2S 니모닉을 함께 표시한 도표.](images/4_9_ko.png)

위 그림에 소개된 EDSAC의 두 어셈블리 명령은 아래와 같이 설명할 수 있다.

- `T 0 S`: T는 명령코드로 A를 의미하는 누산기(accumulator)의 값을 메모리 주소 `0`에 저장한 다음, 누산기를 `0`으로 초기화한다.
- `H 2 S`: H는 명령코드로 메모리 주소 `2`의 값을 곱셈 레지스터(multiplier register) R로 불러와 곱셈 연산을 준비한다.

마지막 `S`는 이 명령이 EDSAC의 짧은 워드 형식(short-word format)을 사용한다는 뜻이다.
...
![프로그래머가 한쪽 판에 T0S와 H2S를 쓰고, 컴퓨터가 다른 판에 대응하는 이진수 명령을 적는다.](images/4_3_ko.png)
...
![프로그래머가 모형 로켓을 옆에 두고 책상에서 종이에 코드를 쓴다.](images/4_7_ko.png)
> "지금은 코딩중"
...
1964~1965년에 MIT Project MAC, 벨 연구소, 제너럴 일렉트릭의 공동 프로젝트로 설계가 시작된 멀틱스(Multics)는 이러한 시분할 방식을 발전시키려 했다. 시분할은 컴퓨터 한 대의 처리 시간을 여러 사용자에게 짧게 나누어 주어, 각자가 자기 터미널에서 대화식으로 작업할 수 있게 하는 방식이다[&lbrack;3&rbrack;][3]. 1970년대에는 화면과 키보드를 갖춘 터미널도 점차 널리 쓰였다. 그렇다면 터미널을 직접 이용할 수 없었던 프로그래머들은 어떻게 코드를 작성하고 결과를 확인했을까?

![프로그래머가 테이프 릴과 화면, 프린터가 달린 대형 컴퓨터 옆에서 키보드를 사용하며 미소 짓는다.](images/4_6_ko.png)
> "드디어 키보드와 모니터가"
...
![손에 든 노란색 천공카드 위에 포트란 프로그래밍용 카드라고 적혀 있다.](images/4_8_ko.png)
...
![프로그래머가 IBM 026 키펀치 기계에 앉아 코드를 천공카드로 옮긴다.](images/4_10_ko.png)
> "코드를 어서 천공 카드에 옮겨야지"
...
![네 명의 프로그래머가 천공카드를 들고 전산실 창구에 줄을 서고, 오퍼레이터가 작업을 접수한다.](images/4_4_ko.png)
> "이거 좀 봐주세요" \
> "다음 사람"

프로그램을 카드나 종이테이프에 천공하기 전이라면, 다른 사람이 종이에 손으로 쓴 프로그램을 그대로 베껴 쓰는 것만으로도 프로그램을 복사할 수 있었다.

![주인공이 옆 사람을 곁눈질하며 천공카드로 옮기기 전의 손글씨 코드를 몰래 베껴 쓴다.](images/4_5_ko.png)
