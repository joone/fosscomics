...
컴퓨터가 처음 만들어졌을 때, 사람들은 어떻게 프로그래밍을 했을까? 초기 컴퓨터는 오늘날과 같은 소프트웨어는 없었고, 탁상 계산기 같아서 릴레이나 진공관 같은 스위치로  논리회로 구성해서 한가지 용도로만 사용할 수 있었다. 에니악(ENIAC)이 개발되면서 프로그램을 재구성할 수 있는데,  플러그판(plugboard)과 스위치를 이용해서 프로그램을 물리적으로 연결하고 데이터와 명령어를 입력하는 방식이였다. 당연히 매우 번거로운 작업이였고, 프로그램을 변경하려면 배선을 다시해야했다[&lbrack;1&rbrack;][1][&lbrack;4&rbrack;][4].
