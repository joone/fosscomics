...
![한 프로그래머가 복잡한 프로젝트를 산에 비유하며 머리를 감싸 쥔다.](images/pdf_complexity_ko.webp)
...
:::panel
![톰프슨이 B라고 적힌 종이를 들고 유닉스 재작성을 제안한다.](images/pdf_b_ko.webp)
> "그래? 데니스, 우리 B언어로 유닉스를 다시 만들어 볼까?" \
> "이제 어셈블리어가 아닌 고급언어로도 OS커널을 만들 수 있을거야."


![리치가 손짓하며 PDP-11에서 B언어가 제대로 동작하도록 만드는 일을 설명한다.](images/pdf_b_first_ko.webp "size:80%")
...
![유닉스 커널 대부분을 C언어로 다시 작성한 뒤 리치가 단말기에 코드를 입력한다.](images/pdf_kernel_ko.webp)
> "ㅎㅎ 이제 어셈블리어로 짤 부분이 훨씬 줄었네."
:::

:::panel
![C 영웅이 UNIX 표지판을 들어 올리고 작은 어셈블리 언어 영웅이 아래에서 바라본다.](images/pdf_hero_ko.webp)
