...
![MIT와 벨 연구소의 프로그래머들이 서로 다른 대형 컴퓨터에서 작업한다.](images/8_15_ko.webp)
> "어디선가 포스가 느껴진다!"
...
![벨 연구소 프로그래머 두 명이 커피를 마시며 멀틱스 프로젝트를 이야기한다.](images/8_1_ko.webp)
> "이제 멀틱스 개발에서 손을 떼야 할 것 같아." \
> "예상보다 너무 개발기간이 길어."

멀틱스 프로젝트는 1964년에 시작되었으나, 코드 크기가 커지고 복잡도가 높아지면서 벨 연구소에서 예상했던 것보다 일정이 많이 지연되고 있었다.

![한 프로그래머가 복잡한 프로젝트를 산에 비유하고, 다른 프로그래머는 머리를 감싸 쥔다.](images/8_4_ko.webp)
...
![벨 연구소 관계자가 떠나는 동안 GE와 MIT 관계자들은 멀틱스라고 적힌 높은 문서 더미 앞에 남아 있다.](images/8_12_ko.webp)
> "우리는 일정을 맞추기가 어려워서 떠납니다."

벨 연구소로 돌아온 켄 톰프슨은 멀틱스 개발 경험을 토대로 좀 더 작고 단순한 운영체제를 만드는 작업을 주도했다.

![켄 톰프슨과 데니스 리치가 작은 전구를 들고 더 단순한 운영체제를 이야기한다.](images/8_11_ko.webp)
> "데니스, 내가 새롭게 운영체제를 만들고 있는데 멀틱스에서 좋은 아이디어를 가지고 오고, 좀 더 작고 단순하게 하려고." \
> "괜찮은 생각인데, 나도 껴줘."

켄 톰프슨은 멀틱스에서 익힌 주요 아이디어를 가져와 더 단순한 형태로 유닉스에 구현했다.

![톰프슨이 명령어 라인, 파일 시스템, 동적 링크라고 적힌 풍선을 들고 있고 멀틱스는 더 많은 풍선을 들고 있다.](images/8_6_ko.webp)
> "무에서 유를 창조할수는 없지"
...
![톰프슨, 리치, 동료 한 명이 파일 시스템의 이름과 디바이스 파일 아이디어를 논의한다.](images/8_14_ko.webp)
> "우선 파일시스템을 만들고 있어." \
> "이름은 유닉스 어때?" \
...
![리치가 새 PDP-11을 소개하자 톰프슨이 어셈블리어 재작성 문제를 설명한다.](images/8_2_ko.webp)
> "켄 PDP-11이 새로 들어왔는데, 여기에 유닉스를 포팅하면 어떨까?" \
> "좋은 생각인데, 문제는 어셈블리어로 다시 코딩을 해야 한다는거지"

![톰프슨이 포팅 완료를 기뻐하고 리치는 새 컴퓨터마다 어셈블리어를 다시 쓸 수 없다고 말한다.](images/8_9_ko.webp)
> "와~ 드디어 포팅 끝!" \
> "그런데, 매번 새 컴퓨터가 나올 때 마다 어셈블리어로 포팅할 수는 없잖아."

하지만 새 컴퓨터가 들어올 때마다 어셈블리 코드를 다시 짤 수는 없는 노릇이었다. 그렇다면 B언어로 유닉스를 다시 만들 수 있을까?

![톰프슨이 B라고 적힌 종이를 들고 유닉스 재작성을 제안하고, 아래에서는 리치가 PDP-11 포팅 문제를 설명한다.](images/8_8_ko.webp)
...
![리치가 PDP-11 단말기에서 B 언어 포팅 작업을 한다.](images/8_5_ko.webp)
...
![톰프슨이 리치에게 B 언어 포팅 진행 상황을 묻는다.](images/8_10_ko.webp)
> "데니스 B언어 포팅은 잘 되가?" \
> "문제가 많아. 우선 PDP-11에서는 문자 데이터 타입에 접근하기가 어려워."

![톰프슨이 B 언어를 완전히 고치자고 제안하자 리치는 다시 짜야 할지도 모른다고 생각한다.](images/8_13_ko.webp)
...
![톰프슨과 리치가 PDP-11 앞에서 새 언어의 이름을 정한다.](images/8_3_ko.webp)
> "C언어라고 부르면 어떨까?" \
> "우와 잘 동작하는데!"
...
![리치가 단말기에 코드를 입력하며 이제 어셈블리어로 코딩하지 않아도 된다고 말한다.](images/8_7_ko.webp)
> "ㅎㅎ 더 이상 어셈블리어로 코딩을 하지 않아도 되네."

![C 영웅이 UNIX 표지판을 들어 올리고 작은 어셈블리 언어 영웅이 아래에서 바라본다.](images/8_16_ko.webp)
