...
![군인과 컴퓨터 조작원이 대형 계산 장치 옆에서 전화로 대화하고 있으며, 뒤에는 미사일 발사대가 놓여 있다.](images/pdf_military_ko.webp)
> "포탄의 탄도 계산은 완료되었나요?" \
> "계산결과는 아직 안나왔어요."
:::
...
![존 모클리와 J. 프레스퍼 에커트가 서로 마주 보고 있으며, 에커트가 손짓하고 있다.](images/pdf_founders_ko.webp "size:90%")
...
![미래에서 온 Joone가 전화로 이야기하고, 다른 직원은 유니박 콘솔 옆에서 출력을 살펴보고 있다.](images/pdf_census_ko.webp)
> "평균 수명 통계는 나왔나요?" \
> "잠깐 기다려보세요"
:::
...
![조지프 매카시 상원의원이 존 모클리를 손가락으로 가리키며 몰아세우고, 모클리는 두 손을 펴 보이며 대응하고 있다.](images/pdf_accusation_ko.webp)
...
![조지프 매카시 상원의원이 존 모클리를 발로 차서 내쫓고 있다.](images/pdf_expulsion_ko.webp "size:90%")
> "빨갱이 어서 꺼져!"
:::
...
:::panel
역사적 기록을 보면 일명 폰 노이만 컴퓨터 아키텍처를 어느 한 사람의 공으로만 돌리기에는 충분치 않다. 이 구조는 에커트와 모클리를 포함한 에니악 팀이 에드박을 개발하는 과정에서 나왔고, 폰 노이만도 컨설턴트로 기여를 했다. 그러나 폰 노이만이 작성한 [「EDVAC에 관한 보고서 초안」(First Draft of a Report on the EDVAC)](https://en.wikipedia.org/wiki/First_Draft_of_a_Report_on_the_EDVAC)에는 저자로 그의 이름만 기재되어 있었고, 이 보고서가 널리 배포되면서 오늘날에도 이 컴퓨터 구조를 폰 노이만 아키텍처라고 부르고 있다.

![폰 노이만이 한 손을 들고 이야기하고 있다.](images/pdf_von_neumann_ko.webp "size:80%")
...
![타임머신을 타고 1950년대로 온 Joone이 대형 IBM 701 컴퓨터의 콘솔을 살펴보고 있다.](images/pdf_ibm701_ko.webp)
> "여전히 진공관을 사용하고, 화면이나 키보드도 없어. 메모리 용량도 36비트 워드 2,048개에 불과해."
:::

:::panel rounded="true"
1954년에 발표된 후속 기종 [IBM 704](https://en.wikipedia.org/wiki/IBM_704)를 위해 [존 배커스](https://en.wikipedia.org/wiki/John_Backus)가 이끄는 IBM 팀은 [포트란](https://en.wikipedia.org/wiki/Fortran)을 개발했다. 포트란은 1953년에 제안되었고 첫 컴파일러는 1957년에 완성되었다. 이 컴파일러는 고급 언어로 작성한 프로그램을 효율적인 기계어로 최적화할 수 있음을 보여주며 컴파일러의 실용성을 프로그래머들에게 입증했다[&lbrack;4&rbrack;][4].

![포트란 개발팀 책임자 존 배커스가 포트란을 설명하며 손짓하고, Joone은 놀란 표정으로 듣고 있다.](images/pdf_fortran_ko.webp)
...
![조작원이 별도의 캐비닛과 카드 장치가 딸린 IBM 650 콘솔 앞에 앉아 있다.](images/pdf_ibm650_ko.webp "<IBM 650>")
:::
...
![도널드 커누스가 IBM 650 컴퓨터의 제어 콘솔 앞에 앉아 있다.](images/pdf_knuth_ko.webp "size:90%")
> "나도 IBM 650으로 처음 프로그래밍을 시작했지"
:::

:::panel
![상상 속 대화에서 Joone은 IBM 650의 크기에 감탄하다가 커누스 옆의 두 번째 캐비닛과 별도의 카드 리더 겸 천공 장치를 발견한다.](images/pdf_card_reader_ko.webp)
> "컴퓨터 크기가 상당히 작아졌어요." \
> "하하, 무슨 소리." \
> "뒤쪽에 장치가 하나 더 있네?" \
> "하하, 이건 카드 리더 겸 천공 장치야."
...
![웃고 있는 트랜지스터들 옆에서 진공관이 은퇴를 이야기하고 있다.](images/pdf_transistors_ko.webp "size:100%")
> "이제 컴퓨터에선 은퇴야... 그동안 너무 혹사당했어."
:::

## 처음으로 대중화에 성공한 컴퓨터
:::panel
1959년에 출시된 IBM 1401 메인프레임은 본격적으로 컴퓨터 시대를 열었다. 1960년대 중반까지 1만 대 이상이 설치되었고, IBM은 1971년에 이 기종을 공식적으로 단종했다. 컴퓨터 업계의 포드 모델-T라고 부를 정도로 상업용 컴퓨터 대중화에 성공했다[&lbrack;8&rbrack;][8]. 우리나라에서도 1967년에 최초로 도입한 컴퓨터였다[&lbrack;9&rbrack;][9].
![여성이 IBM 1401 메인프레임 옆에 서 있다.](images/pdf_feature_ko.webp "<IBM 1401> size:80%")
:::
...
8. [IBM 1401, IBM 역사](https://www.ibm.com/history/1401)
9. [초기 컴퓨터의 진화 (보관본), LG CNS 블로그](https://web.archive.org/web/20210923140825/https://blog.lgcns.com/1042)
...
[8]: https://www.ibm.com/history/1401 "IBM 1401, IBM 역사"
[9]: https://web.archive.org/web/20210923140825/https://blog.lgcns.com/1042 "초기 컴퓨터의 진화, LG CNS 블로그 (보관본)"

# 더 읽을 글

- [초기 컴퓨터의 진화 (보관본)](https://web.archive.org/web/20210923140825/https://blog.lgcns.com/1042), LG CNS 블로그
